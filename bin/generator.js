...
const enumValueToObject = (enumMap, prop) => (value, index) => ({
...
const populateEnumValues = (enums, prop, enumeration) => {
    const enumMap = getEnumMapFrom(prop.description);

    if (enums.find(existingEnum => existingEnum.name === enumeration.name)) {
        return;
    }
    enumeration.values = prop.enum.map(enumValueToObject(enumMap, prop));
...
const Generator = (function () {
    ...
    Generator.prototype.createMustacheViewModel = function () {
            ...
            _.forEach(api, function (op, m) {
                ...
                _.forEach(params, function (parameter) {
                    ...
                    if (parameter.enum) {
                        ...
                        } else if (['string', 'number'].includes(parameter.typescriptType)) {
                            const enumeration = {name: null, values: []};

                            // upper keyword to templates
                            enumeration.upper = UPPERCASE_RENDERER;
                            enumeration.name = parameter.name[0].toUpperCase() + parameter.name.substring(1);
                            populateEnumValues(data.enums, parameter, enumeration);
        ...
        _.forEach(swagger.definitions, function (defin, defVal) {
            ...
            definition.lower = LOWERCASE_RENDERER;
            ...
            _.forEach(defin.properties, function (propin, propVal) {
                ...
                if (property.isArray) {
                    ...
                    populateEnumValues(definition.enums, propin, enumeration);
                    definition.hasEnums = true;

                    const existingEnumIndex = data.enums.findIndex(
                        existingEnum => existingEnum.name === enumeration.name
                    );
                    if (existingEnumIndex >= 0) {
                        data.enums.splice(existingEnumIndex, 1);
                    }
                }
                else {
